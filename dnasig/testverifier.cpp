...
//    qDebug() << "Releasing"<< pubKeys.count() << "and"
//             << privKeys.count() << "keys";
...
bool TestVerifier::writeKeys(EVP_PKEY *pkey, QString k, QString p, X509 *x509, QString c)
{
    ...
    if(!pkey_file)
    {
        qWarning("Unable to open \"%s\" for writing.", QC_STR(k));
        return false;
    }
    ...
    if(!x509_file)
    {
        qWarning( "Unable to open \"%s\" for writing.", QC_STR(c) );
        return false;
    }
    ...
    if(!pub_file)
    {
        qWarning("Unable to open \"%s\" for writing.", QC_STR(p));
        return false;
    }
