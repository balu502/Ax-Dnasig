    ...
    , verifier  (new TestVerifier())
{
    ...
    qDebug()<< "Verifier status -"  << !vic;
    qDebug()<< "Current path -"     << QDir::currentPath();
    qDebug()<< "Lib paths -"        << qApp->libraryPaths();
...
void DnaSig::generateKeys(){
    ...
    && (QMessageBox::No ==
            QMessageBox::warning(sDialog, sDialog->windowTitle(),
                tr("Existing private key %1 will no longer used. Proceed?")
                        .arg(verifier->privKeyList().first()),
                QMessageBox::Yes|QMessageBox::No)))
        return;


    if (edD->text().trimmed().isEmpty()
    || !edD->text().toInt()){
        QMessageBox::warning(sDialog, sDialog->windowTitle(),
                tr("Please input correct %1").arg(edD->toolTip()));
        return;
    }

    if (edC->text().trimmed().isEmpty()){
        QMessageBox::warning(sDialog, sDialog->windowTitle(),
                tr("Please input correct %1").arg(edC->toolTip()));
        return;
    }

    if (edCN->text().trimmed().isEmpty()){
        QMessageBox::warning(sDialog, sDialog->windowTitle(),
                tr("Please input correct %1").arg(edCN->toolTip()));
        return;
    }

    if (edO->text().trimmed().isEmpty()){
        QMessageBox::warning(sDialog, sDialog->windowTitle(),
                tr("Please input correct %1").arg(edO->toolTip()));
        return;
    }

    QString pubName = QString("%1_pubkey.pem"   ).arg((int)verifier),
            privName= QString("%1_privkey.pem"  ).arg((int)verifier),
            crtName = QString("%1_privkey.cer"  ).arg((int)verifier);
    ...
    for( int i=0; i < tItems.count(); i++)
        if ("item" == tItems.at(i).nodeName()){
            ...
            if( applySign ){
                ...
                qDebug() << "Apply signate result is - " << !est;
                scnt += est;
            }
        }
...
bool DnaSig::loadTest( QDomNode * tNode ){
    ...
    foreach( kf, verifier->pubKeyList() )
        if ( verified = verifier->isValidated(tString, kf) ){
            ...
            qDebug() << tName << "Signed with key" << kf;
            break;
        }
